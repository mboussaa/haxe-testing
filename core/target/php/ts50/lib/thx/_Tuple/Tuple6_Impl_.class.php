<?php

// Generated by Haxe 3.3.0
class thx__Tuple_Tuple6_Impl_ {
	public function __construct(){}
	static function of($_0, $_1, $_2, $_3, $_4, $_5) {
		return _hx_anonymous(array("_0" => $_0, "_1" => $_1, "_2" => $_2, "_3" => $_3, "_4" => $_4, "_5" => $_5));
	}
	static function _new($_0, $_1, $_2, $_3, $_4, $_5) {
		return _hx_anonymous(array("_0" => $_0, "_1" => $_1, "_2" => $_2, "_3" => $_3, "_4" => $_4, "_5" => $_5));
	}
	static function flip($this1) {
		return _hx_anonymous(array("_0" => $this1->_5, "_1" => $this1->_4, "_2" => $this1->_3, "_3" => $this1->_2, "_4" => $this1->_1, "_5" => $this1->_0));
	}
	static function dropLeft($this1) {
		return _hx_anonymous(array("_0" => $this1->_1, "_1" => $this1->_2, "_2" => $this1->_3, "_3" => $this1->_4, "_4" => $this1->_5));
	}
	static function dropRight($this1) {
		return _hx_anonymous(array("_0" => $this1->_0, "_1" => $this1->_1, "_2" => $this1->_2, "_3" => $this1->_3, "_4" => $this1->_4));
	}
	static function toString($this1) {
		$tmp = Std::string($this1->_0);
		$tmp1 = "Tuple6(" . _hx_string_or_null($tmp) . ",";
		$tmp2 = Std::string($this1->_1);
		$tmp3 = _hx_string_or_null($tmp1) . _hx_string_or_null($tmp2) . ",";
		$tmp4 = Std::string($this1->_2);
		$tmp5 = _hx_string_or_null($tmp3) . _hx_string_or_null($tmp4) . ",";
		$tmp6 = Std::string($this1->_3);
		$tmp7 = _hx_string_or_null($tmp5) . _hx_string_or_null($tmp6) . ",";
		$tmp8 = Std::string($this1->_4);
		$tmp9 = _hx_string_or_null($tmp7) . _hx_string_or_null($tmp8) . ",";
		$tmp10 = Std::string($this1->_5);
		return _hx_string_or_null($tmp9) . _hx_string_or_null($tmp10) . ")";
	}
	static function arrayToTuple6($v) {
		return _hx_anonymous(array("_0" => $v[0], "_1" => $v[1], "_2" => $v[2], "_3" => $v[3], "_4" => $v[4], "_5" => $v[5]));
	}
	function __toString() { return 'thx._Tuple.Tuple6_Impl_'; }
}
